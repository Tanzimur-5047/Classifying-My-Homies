...
CLASS_NAMES = ['Aninda', 'Himel', 'Sukomal']
...
if uploaded_file is not None:
    ...
    if len(faces) == 0:
        ...
        recognized_friends = [r for r in results if r['name'] is not None]

        if len(recognized_friends) == 1:
            friend = recognized_friends[0]
            st.success(f"### 👤 {friend['name']} detected!")
            st.metric("Confidence", f"{friend['confidence']:.1%}")
            st.divider()
            st.markdown(f"**Playing media for {friend['name']}:**")
            play_media(friend['name'])

        elif len(recognized_friends) > 1:
            annotated = image_bgr.copy()

            for r in recognized_friends:
                x, y, w, h = r['box']
                annotated = draw_box(annotated, x, y, w, h, r['name'], r['confidence'])

            annotated_rgb = cv2.cvtColor(annotated, cv2.COLOR_BGR2RGB)
            st.image(annotated_rgb, caption="Detected Friends", use_container_width=True)

            st.markdown("### 👥 Friends found in this photo:")
            for r in recognized_friends:
                st.success(f"**{r['name']}** — {r['confidence']:.1%} confidence")

 
        else:
            st.warning("😕 Faces detected but none of my friends recognized.")
            st.caption("If this should be one of your friends, the confidence was below 70%.")